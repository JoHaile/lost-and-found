...
2. **Weighted score (0–100)** across six signals:
   ...
   | Name        | 25     | Best of Jaro-Winkler and stemmed token overlap, computed on names reduced to identity words: color words, the item's own location/color field words, and filler/function words ("near", "with", "lost"…) are all stripped, leaving item nouns so those signals can't inflate or dilute the name score. Example: "Black backpack near the fountain" → "backpack"; "Backpack with sketchbooks" → "backpack sketchbooks" |
